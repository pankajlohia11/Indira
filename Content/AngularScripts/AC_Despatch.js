app1.controller("AC_DespatchCtrl", function ($scope, $http, $window, $compile) {
    ...
    $scope.RedirectDestoInvoice = function (a) {
        window.location = '../ET_Sales_OneToManyInvoice/ET_Sales_DespatchtoOneToManyInvoice?type=' + $scope.Type + '&DespatchId=' + a;
    }
    ...
    function validate() {
        ...
        for (i = 0; i < tr.length; i++) {
            var td = $(tr[i]).find("td");
            var ordqty = $(td[5]).find("input").val();
            var avlqty = $(td[6]).find("input").val();
            var stkqty = $(td[7]).find("input").val();
            var desqty = $(td[8]).find("input").val();
            if (desqty == "") {
    ...
    $scope.Restoredeleterecords = function (a, $event, b) {
        ...
        //alert(a.toString());
        if(b) {
            alertmessageModified($event, a.toString(),b,'ET_Sales_Despatch', 'ET_Sales_Despatch_RestoreDelete');
         }
        else
        {
    ...
    $scope.PrintRecords = function (a,b) {
        ...
        if ($scope.Isview) {
            ...
            var post1 = $http({
                method: "POST",
                url: "/ET_Sales_Despatch/ET_Sales_DespatchPrint",
                dataType: 'html',
                data: {
                    id: a,
                    lang: b
            ...
            post1.success(function (data, status) {
                ...
                if (data == "Session Expired") {
                    ...
                    $("#PAthid").attr("href", "/Sales/PDFList/Despatch/" + data[0].D_Code + "/" + data[0].D_Code + ".pdf ");
                    $("#PAthid").text(data);
                    $("#div_View").css("display", "none");
                    $("#div_Restore").css("display", "none");
                    $("#div_Edit").css("display", "none");
    ...
    $scope.MailSentForOrder = function (a) {
        ...
        $.ajax({
            type: "GET",
            url: '/ET_Sales_Despatch/ET_Sales_Mail_Compose_Reply_Forward',
            data: {
                id: a
            },
