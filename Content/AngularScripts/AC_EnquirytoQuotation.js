app1.controller("AC_EnqtoQuotationCtrl", function ($scope, $http, $window, $compile) {
    function GetTypeFromURL() {
        var type1 = window.location.href.toString().split("&");
        var enqid = window.location.href.toString().split("EnqId=");
    ...
    function GetPrivilages() {
        ...
        getprivilages.then(function (privilage1) {
            ...
            GetCustomerList();
            PaymentTerms();
            GetEnquiriesDetails($scope.EnqID);
        }, function () {
            alert('Privilages Not Found');
        }
        );
    }
    function GetEnquiriesDetails(a) {
        var getcustomerlist = $http.get("/ET_Sales_Quotation/GetEnquiriesDetails",
            {
                params: { id: a }
            });
        getcustomerlist.then(function (customerlist) {
            ...
            $scope.Q_CustomerID = res[0].E_CustomerID;
            $scope.Q_EnquiryID = res[0].E_ID;
            $("#drpCustomer").val($scope.Q_CustomerID).trigger("chosen:updated");
            $scope.CustomerChange();
            $scope.EnquiryChange();
            $("#drpEnquiry").val($scope.Q_EnquiryID).trigger("chosen:updated");
    ...
    $scope.showRecords = function () {
        window.location = '../ET_Sales_Enquiry/ET_Sales_Enquiry?type=' + $scope.Type;
    }
    ...
    $scope.$watch("enquirydata1", function (value) {
        ...
        if (val) {
            ...
            setTimeout(function () {
                ...
                for (var i = 0; i < tr.length; i++) {
                    var td = tr[i].cells;
                    $(td[0]).find("select").val(enquirydata[i].QD_ProductID).trigger("chosen:updated");
                    $(td[0]).find("p").text(enquirydata[i].QD_ProductID);
                    $(td[1]).find("input").val(enquirydata[i].QD_UOM);
                    if (enquirydata[i].QD_Unit_Price == "")
                        enquirydata[i].QD_Unit_Price = "0";
                    $(td[2]).find("input").val(parseFloat(enquirydata[i].QD_Unit_Price).toLocaleString("es-ES", { minimumFractionDigits: 2 }));
                    $(td[3]).find("input").val(parseFloat(enquirydata[i].QD_Quantity).toLocaleString("es-ES", { minimumFractionDigits: 2 }));
                    $(td[4]).find("input").val(parseFloat(enquirydata[i].QD_Unit_Price * enquirydata[i].QD_Quantity).toLocaleString("es-ES", { minimumFractionDigits: 2 }));
