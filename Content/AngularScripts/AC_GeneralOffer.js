app1.controller("AC_GeneralOfferCtrl", function ($scope, $http, $window, $compile) {
   ...
   //View the records
    ...
    // Restore div in General OfferRedirectofferToOrder
    ...
    $scope.RedirectofferToOrder = function (a) {
        window.location = '../ET_Sales_OrderDetails/ET_Sales_OffertoOrderDetails?type=' + $scope.Type + '&offerId=' + a;
           // '@Url.Action("ET_Sales_OffertoOrderDetails", "ET_Sales_OrderDetails", new { type = ' + $scope.Type + ', offerId = ' + a + '})';
    ...
    function GetAC_GetOffersList()
    {
        ...
        var getac_offerlist = $http.get("/ET_Admin_GeneralOffer/GetOffersList",
            {
                params: { deleted: false, type:filterOption }
            });
    ...
    $scope.PrintRecords = function (a,b) {
        ...
        if ($scope.Isview) {
            ...
            var post1 = $http({
                method: "POST",
                url: "/ET_Admin_GeneralOffer/ET_Admin_GeneralOfferPrint",
                dataType: 'html',
                data: {
                    id: a,
                    lang: b
            ...
            post1.success(function (data, status) {
                ...
                if (data == "Session Expired") {
                    ...
                    $("#PAthid").attr("href", "/Sales/PDFList/SupplierOffer/" + data[0].OFFER_CODE + "/" + data[0].OFFER_CODE + ".pdf ");
                    $("#PAthid").text(data);
                    $("#div_View").css("display", "none");
                    $("#div_Restore").css("display", "none");
                    $("#div_Edit").css("display", "none");
    ...
    $scope.Restoredeleterecords = function (a, $event, b) {
        ...
        //alert(a.toString());
        if (b) {
            alertmessageModified($event, a.toString(), b, 'ET_Admin_GeneralOffer', 'ET_Admin_GeneralOffer_Restore_Delete');
        }
        else
        {
        ...
        if (val) {
            ...
            setTimeout(function () {
                ...
                for (var i = 0; i < tr.length; i++)
                {
                    ...
                    $(td[1]).find("p").text(contactdata[i].PRODUCT_ID).trigger("chosen:updated");
                    $(td[2]).find("input").val(contactdata[i].CATEGORY_NAME);
                    $(td[3]).find("input").val(contactdata[i].PRODUCT_NAME);
                    $(td[4]).find("input").val(contactdata[i].UOM_NAME);
                    $(td[5]).find("select").val(contactdata[i].CURRENCY_ID).trigger("chosen:updated");
                    $(td[6]).find("input").val(parseFloat(contactdata[i].PRICE).toLocaleString("es-ES", { minimumFractionDigits: 2 }));
                    $(td[7]).find("input").val(parseFloat(contactdata[i].QUANTITY).toLocaleString("es-ES", { minimumFractionDigits: 2 }));
                }
            }, 100);
        }
    });
