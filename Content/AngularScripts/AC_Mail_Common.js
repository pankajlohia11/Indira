app1.controller("AC_Mail_Common", function ($scope, $http, $window, $compile)
{
    alert("Mail View area to be used");
    mailList();
    $scope.deleteFiles = function ($event, b) {
        debugger;
        $($event.target.parentElement.parentElement).fadeOut(800, function () { $(this).remove(); });
    };

    $scope.compose = function (reply, body, cc, subject, ToAdd, attachedFile, rly_all_fwd)
    {
        $scope.MST_To = "";
        $('#mailIds').tagsinput('removeAll');
        $('#mailCcIds').tagsinput('removeAll');
        $scope.MST_Cc = "";
        $scope.MST_Subject = "";
        $scope.MST_Body = "";
        ...
        if (rly_all_fwd == 1 || rly_all_fwd == 2)
            subject = "RE: " + subject;
        else
            subject = "FWD: " + subject;

        //var reply = reply.split(',');
        //var ToAdd = ToAdd.split(',');
        //var cc = cc.split(',');

        if (rly_all_fwd != 3) {
            for (var i = 0; cc.length > i; i++) {
                $('#mailIds').tagsinput('add', reply[i]);
            }
            if (rly_all_fwd == 2) {
                for (var i = 0; cc.length > i; i++) {
                    $('#mailIds').tagsinput('add', ToAdd[i]);
                }


                for (var i = 0; cc.length > i; i++) {
                    $('#mailCcIds').tagsinput('add', cc[i]);
                }
            }
        }
        ...
        $scope.MST_Body = body;
        $scope.attachedFileFwd = attachedFile;
    ...
    //Single mail sending
    $scope.singleMail = function (MIID, MI_Subject, MI_Address, MI_Message, MI_Date, MI_Attachment, MI_cc, MI_To) {
        $('#SingleSentMail').css('display', 'none');
        $('#Compose').css('display', 'none');
        $('#Inbox').css('display', 'none');
