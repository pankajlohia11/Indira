app1.controller("AC_PackinglistCtrl", function ($scope, $http, $window, $compile)
{
    GetPrivilages();
    GetPackingList();
    GetProducts(); 
    ...
    function GetCustomerList() {
        var getcustomerlist = $http.get("/ET_Sales_PackingList/GetCustomers",
            {
                params: {
                    id: $scope.PL_ID,
                }
    ...
    $scope.Restoredeleterecords = function (a, $event, b) {
        if (b) {
            alertmessageModified($event, a.toString(),b,'ET_Sales_PackingList', 'ET_Master_Store_RestoreDelete');
        }
        else 
        {
    ...
    $scope.PrintRecords = function (a,b) {
        ...
        if ($scope.Isview) {
            var post = $http({
                method: "POST",
                url: "/ET_Sales_PackingList/ET_Master_PackingList_Print",
                dataType: 'html',
                data: {
                    id: a,
                    lang: b
