app1.controller("AC_ScheduleCtrl", function ($scope, $http, $window, $compile) {
    ...
    function validate()
    {
        ...
        if ($("#txtSchDate").val() == "")
        {
            message = "Select Customer Schedule Date";
            toastr["error"](message, "Notification");
            return "";
        }
        if ($("#txtSupSchDate").val() == "") {
            message = "Select Supplier Schedule Date";
            toastr["error"](message, "Notification");
            return "";
        }
        ...
        if ($scope.Isedit) {
            ...
            post.success(function (data, status) {
                if (data == "Session Expired") {
                    ...
                    var SUPSHDATE = new Date(parseInt(QuotationByID.SUP_SH_DATE.substr(6)));
                    var Orderdt = ("0" + SHDATE.getDate()).slice(-2) + "-" + ("0" + (SHDATE.getMonth() + 1)).slice(-2) + "-" + SHDATE.getFullYear();
                    var suppDeliveryDate = ("0" + SUPSHDATE.getDate()).slice(-2) + "-" + ("0" + (SUPSHDATE.getMonth() + 1)).slice(-2) + "-" + SUPSHDATE.getFullYear();
                    
                    $("#txtSchDate").val(Orderdt);
                    $("#txtSupSchDate").val(suppDeliveryDate);
    ...
    $scope.RedirectschduletoShipment = function (a) {
        window.location = '../ET_Sales_Shipment/ET_Sales_SchduletoShipment?type=' + $scope.Type +'&SchduleID='+a;
    }
    
    $scope.RedirectschduletoDespatch = function (a) {
        window.location = '../ET_Sales_Despatch/ET_Sales_OrderToDespatch?type=2&OrderId=' + a;
    }
    $scope.RedirectToUpsLink = function (a) {
        window.open('https://www.campusship.ups.com/cship/create?loc=de_DE','_blank');
    }
    ...
    $scope.Restoredeleterecords = function (a, $event, b) {
        if (b)
        {
            alertmessageModified($event, a.toString(),b,'ET_Sales_Shedule', 'ET_Sales_Schedule_RestoreDelete');
        }
        else
        {
