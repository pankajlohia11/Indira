app1.controller("AC_ShipmentCtrl", function ($scope, $http, $window, $compile)
{
    ...
    function validate() {
        ...
        {
            var td = $(tr[i]).find("td");
            var scheduledate = $(td[5]).find("input").val();
            var availableqty = 0;
            var availableQtyNum = 0;
            var qty = 0;
            var shipmentQty = 0;
            if ($scope.S_Type == 2)
            {
                availableqty=$(td[9]).find("input").val();
                qty= $(td[10]).find("input").val();
            }
            else
            {
            ...
            shipmentQty = parseFloat(qty.split('.').join("").replace(",", "."));
            availableQtyNum = parseFloat(availableqty.split('.').join("").replace(",", "."));
            if (scheduledate == "")
            {
            ...
            if (shipmentQty > 0) {
                if (shipmentQty > availableQtyNum) {
                    //if ($scope.S_Type =="3") {
    ...
    function GetOrders() {
        if ($scope.S_Type != "")
        {
            var getorderlist = $http.get("/ET_Sales_Shipment/GetScheduledOrders",
                {
                    params: {
                        type: $scope.S_Type,
                        scheduleCode: $scope.S_ScheduleID
                    }
                });
        ...
        if ($scope.Isedit) {
            ...
            getshipmentlist.then(function (shipmentlist) {
                ...
                if (res.isfab == 0) {
                    ...
                    Rowhtml = "<tr><td colspan='2' style='text-align:center;font-weight:bold;'>Total</td><td style='text-align:right;font-weight:bold;'>" + parseFloat(res.popupInvoiceAmt[0]).toLocaleString("es-ES", { minimumFractionDigits: 2 }) + " €<input style='display:none;' type='text' id='totalInvoiceamount' value='" + parseFloat(res.popupInvoiceAmt[0]).toLocaleString("es-ES", { minimumFractionDigits: 2 }) + "' /></td><td style='text-align:right;font-weight:bold;'>" + parseFloat(res.popupCommissionAmt).toLocaleString("es-ES", { minimumFractionDigits: 2 }) + " €</td></tr>";
                    ...
                    for (var i = 0; i < res.data.length; i++)
                    {
                        Rowhtml = "<tr><td>" + res.data[i].ordNo + "</td><td>" + parseFloat(res.data[i].commission).toFixed(0) + " %</td><td style='text-align:right'>" + parseFloat(res.FabAmounts[i]).toLocaleString("es-ES", { minimumFractionDigits: 2 }) + " €</td><td style='text-align:right'>" + parseFloat(res.data[i].commission * res.FabAmounts[i] / 100).toLocaleString("es-ES", { minimumFractionDigits: 2 }) + " €</td></tr>";
                        temp = $compile(Rowhtml)($scope);
                        angular.element(document.getElementById('popupCommisionTable')).append(temp);
                        totfab = totfab + (parseFloat(res.FabAmounts[i]));
                    }
                    Rowhtml = "<tr><td colspan='2' style='text-align:center;font-weight:bold;'>Total</td><td style='text-align:right;font-weight:bold;'>" + totfab.toLocaleString("es-ES", { minimumFractionDigits: 2 }) + " €<input style='display:none;' type='text' id='totalInvoiceamount' value='" + totfab.toLocaleString("es-ES", { minimumFractionDigits: 2 }) + "' /></td><td style='text-align:right;font-weight:bold;'>" + parseFloat(res.popupCommissionAmt).toLocaleString("es-ES", { minimumFractionDigits: 2 }) + " €</td></tr>";
                    temp = $compile(Rowhtml)($scope);
                    angular.element(document.getElementById('popupCommisionTable')).append(temp);
    ...
    $scope.ShipmenttoPackingList = function (a)
    {
        window.location = '../ET_Trading_PackingList/ET_Trading_ShipmentToPackingList?&ShipID=' + a;
    }
