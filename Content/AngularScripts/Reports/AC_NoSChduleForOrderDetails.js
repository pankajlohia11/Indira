app1.controller("AC_NoSchduleForOrderDetailsCtrl", function ($scope, $http, $window, $rootScope) {
    debugger;
    ...
    $("#div_treereport").html("");
    var content = "<tr><td colspan='10' style='text-align:center;color:red;'>Apply Filters</td></tr>";
    ...
    function GetCustomerSupplierSalesperson() {
        $http({
            method: 'GET',
            url: '/PendingShipmentDetails/GetCustomerSupplierSalesperson',
    ...
    $scope.GetAgencyShipmentDetails = function () {
        ...
        if ($scope.SelectedDuration == "7" && ($("#FromDate").val() == "" || $("#ToDate").val() == "")) {
            ...
            debugger;
            var getAgencyShipmentDetails = $http.get("/NoSchduleForOrder/GetAgencyShipmentDetails",
                {
                    params:
                        {
                ...
                if (res.data.indexOf("Validation") > -1) {
                    ...
                    if (result.length > 0) {
                        ...
                        for (var i = 0; i < result.length; i++) {
                            if ($scope.SelectedOredertype == 3 || $scope.SelectedOredertype == 2 || $scope.SelectedOredertype == 1) {
                                content = content + "<tr data-tt-id='" + result[i].ID + "' data-tt-parent-id=" + result[i].PID + ">";
                                content = content + "<td>" + result[i].customerName + "</td>";
                                content = content + "<td>" + result[i].Order_No + "</td>";
                                content = content + "<td>" + result[i].Order_Date + "</td>";
                                ...
                                if (result[i].Unit_Price == "") {
                                    content = content + "<td style='text-align:right;'></td>";
                                }
                                else {
                                    content = content + "<td style='text-align:right;'>" + parseFloat(result[i].Unit_Price).toLocaleString("es-ES", { minimumFractionDigits: 2 }) + "</td>";
                                }
                                if (result[i].Unit_Price == "") {
                                    content = content + "<td style='text-align:right;'></td>";
                                }
                        ...
                        var content = "<tr><td colspan='10' style='text-align:center;color:red;'>No Data Found</td></tr>";
                        $("#div_treereport1").html(content);
                    }
