...
namespace Euro.Controllers.Sales
{
    public class ET_Sales_PackingListController : Controller
    {
        ...
        public JsonResult GetCustomers(int id)
        {
            ...
            if (val)
            {
                try
                {
                    ...
                    {
                        var custs = dbcontext.Tbl_Packing_List.Single(a => a.PL_ID == id).PL_CustomerID;
        ...
        public JsonResult GetOrderList(int custid)
        {
            ...
            if (val)
            {
                try
                {
                    ...
                    {
                        dbcontext.Configuration.ProxyCreationEnabled = false;
                        var StoreUser = dbcontext.Tbl_Master_Order.Where(m => m.COM_KEY == comp_key && m.SO_OrderType == 3 && m.SO_CutomerID == custid).ToList();
        ...
        [HttpPost]
        public JsonResult ET_Master_Store_Add(int PL_ID, string PL_Code, string PL_Date,int PL_CustomerID, string PL_OrderNo, string PL_Remarks, string EnquiryDetails)
        {
            bool val = Session["UserID"] == null ? false : true;
            if (val)
            {
            ...
            if (val)
            {
                try
                {
                    ...
                    if (lang == "E")
                        ...
                        return PartialView("/Views/Sales/ET_Sales_PackingList/ET_Sales_PackingList_Print_German.cshtml", obj);
                }
                catch (Exception exe)
                {
                    string actionName = this.ControllerContext.RouteData.Values["action"].ToString();
