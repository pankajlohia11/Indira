...
namespace DataAccess.Admin_DA
{
    public class ET_Admin_Roles_DL
    {
        ...
        public decimal Tbl_Master_Role_Add(Tbl_Master_Role _tbl_Master_Role, bool automanual, string prefix, string ChildData, out string rolecode)
        {
            decimal result = 0;
            ...
            try
            {
                if (_tbl_Master_Role.ROLE_ID == 0)
                {
                    ...
                    context.Tbl_Master_Role.Add(Objtmr);
                    ...
                    if (automanual == true)
                    {
                        ...
                        Tbl_Master_Role Objtbl_Master_Role = context.Tbl_Master_Role.Single(m => m.ROLE_ID == Objtmr.ROLE_ID);
                        {
                    ...
                    for (int i = 0; i < ChildRow.Length - 1; i++)
                    {
                        ...
                        Tbl_AccessPermissions objAccess = new Tbl_AccessPermissions()
                        {
                            ...
                            IS_ADD = Convert.ToBoolean(Convert.ToInt32(ChildRecord[2].ToString()) == 0 ? false : true),
                            IS_EDIT = Convert.ToBoolean(Convert.ToInt32(ChildRecord[3].ToString()) == 0 ? false : true),
                            IS_DELETE = Convert.ToBoolean(Convert.ToInt32(ChildRecord[4].ToString()) == 0 ? false : true),
                            IS_FULLCONTROL = Convert.ToBoolean(Convert.ToInt32(ChildRecord[5].ToString()) == 0 ? false : true),
                            IS_VIEW = Convert.ToBoolean(Convert.ToInt32(ChildRecord[6].ToString()) == 0 ? false : true),
                            CREATED_BY = 1,
                            CREATED_DATE = DateTime.Now,
                            DELETED = false
                        };
                        context.Tbl_AccessPermissions.Add(objAccess);
                    }
                    if (context.SaveChanges() != ChildRow.Length - 1)
                    {
                        success = false;
                    }
                    result = Objtmr.ROLE_ID;
                    rolecode = Objtmr.ROLE_CODE;
                }
                else
                {
                    Tbl_Master_Role Objtbl_Master_Role = context.Tbl_Master_Role.Single(m => m.ROLE_ID == _tbl_Master_Role.ROLE_ID);
                    {
                    ...
                    result = Objtbl_Master_Role.ROLE_ID;
                    rolecode = Objtbl_Master_Role.ROLE_CODE;
                    if (context.SaveChanges() == 0)
                    {
                        success = false;
                    }
                    Tbl_AccessPermissions objAccess1 = new Tbl_AccessPermissions();
                    context.Tbl_AccessPermissions.RemoveRange(context.Tbl_AccessPermissions.Where(x => x.ROLE_ID == Objtbl_Master_Role.ROLE_ID));
                    context.SaveChanges();
                    string[] ChildRow = ChildData.Split('|');
                    for (int i = 0; i < ChildRow.Length - 1; i++)
                    {
